...
- **List external PRs**: use `gh pr list --state open --json number,title,body,labels,author,authorAssociation,comments`. Keep `CONTRIBUTOR`, `FIRST_TIME_CONTRIBUTOR`, and `NONE`.
- **Comment, label, or close**: use `gh pr comment`, `gh pr edit`, and `gh pr close`.

GitHub shares one number space across issues and PRs. For a reference such as `#42`, try `gh pr view 42`, then `gh issue view 42`.
...
- **Blocking**: use GitHub issue dependencies. If unavailable, add `Blocked by: #<n>` to the child.
...
- **Resolve**: comment with the answer, close the issue, and add a context link to the map.
