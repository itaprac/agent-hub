...
def test_fresh_repository_reports_every_target_as_missing(
    ...
    assert levels[f"claude project demo/beta: {project}/.claude/skills/beta"] == "MISSING"
    assert levels[f"claude global: {home}/.claude/CLAUDE.md"] == "MISSING"
    assert levels[f"claude project demo: {project}/CLAUDE.md"] == "MISSING"
...
def test_project_without_a_path_for_this_machine_is_skipped(content: Path) -> None:
    report = core.status(content)
    levels = by_text(report)
    assert levels[f"project absent: no path for machine '{MACHINE_ID}'"] == "skip"


def test_skipped_projects_come_first(content: Path) -> None:
    report = core.status(content)
    assert report.checks[0].kind == "project"
    assert report.checks[0].level == "skip"
...
def test_edited_managed_block_is_stale(content: Path, home: Path) -> None:
    ...
    check = find(core.status(content), target)
...
def test_checks_carry_target_metadata(content: Path, project: Path) -> None:
    report = core.status(content)
    beta = next(check for check in report.checks if check.name == "beta")
    assert beta.kind == "skill"
    assert beta.agent == "claude"
    assert beta.project == "demo"
    assert beta.target == str(project / ".claude" / "skills" / "beta")
...
def test_status_rejects_an_unknown_machine(content: Path) -> None:
    (content / "config" / "hub.toml").write_text(
        '[machines]\nunused-host = "other"\n', encoding="utf-8"
    )
    with pytest.raises(config.ConfigError):
        core.status(content)
...
def test_configuration_errors_become_one_error_check_with_exit_two(content: Path) -> None:
    report = core.config_error_report(content, "config/agents.toml: key 'claude.mode': bad")
...
def test_configuration_error_text_stays_on_one_line(content: Path) -> None:
    report = core.config_error_report(content, "first line\nsecond line")
