...
def test_launchd_timer_plist_uses_resolved_commands_and_store_environment(content, home, manager):
    ...
    assert document["ProgramArguments"] == [str(binary / "agent-hub"), "sync", "--quiet"]
    ...
    assert document["StandardErrorPath"] == str(home / "Library/Logs/agent-hub-sync.error.log")
...
def test_launchd_on_off_and_status_are_idempotent(content, home, manager):
    ...
    assert_ok(services.timer("on", content))
    assert manager.calls[before:] == [["launchctl", "print", "gui/501/com.agenthub.sync"]]
...
def test_launchd_reloads_when_store_changes(content, home, tmp_path, manager):
    ...
    assert [call[1] for call in manager.calls[before:]] == ["print", "bootout", "bootstrap"]
    document = plistlib.loads((home / "Library/LaunchAgents/com.agenthub.sync.plist").read_bytes())
...
def test_launchd_ui_uses_fixed_local_bind_and_separate_logs(content, home, manager):
    ...
    document = plistlib.loads((home / "Library/LaunchAgents/com.agenthub.web.plist").read_bytes())
    assert document["ProgramArguments"][1:] == ["--host", "127.0.0.1", "--port", "7337", "--quiet"]
...
def test_launchctl_failure_is_reported_and_failed_off_keeps_plist(content, home, manager):
...
def test_ui_start_failure_uses_only_fresh_tcc_errors(content, home, manager, monkeypatch, fresh_denial):
    write(home / "Library/Logs/agent-hub-web.error.log", services.FILE_ACCESS_DENIAL_MARKER + "\n")
...
def test_service_paths_do_not_follow_symlinks(content, home, tmp_path, manager, monkeypatch, system):
...
def test_resolved_npx_directory_is_kept_in_service_path(content, home, tmp_path, manager):
...
def test_systemd_timer_files_and_commands(content, home, manager, monkeypatch):
    ...
    assert ["systemctl", "--user", "enable", "--now", "agent-hub-sync.timer"] in manager.calls
    status = services.timer("status", content)
    assert_ok(status)
...
def test_systemd_quotes_paths_without_shell_or_environment_expansion(content, home, manager, monkeypatch):
    monkeypatch.setattr(services.platform, "system", lambda: "Linux")
    ...
    environment = next(line for line in text.splitlines() if line.startswith('Environment="PATH='))
...
def test_off_does_not_require_entrypoints_or_store_to_exist(home, tmp_path, manager, monkeypatch):
    monkeypatch.setenv("PATH", str(tmp_path / "empty"))
    assert_ok(services.timer("off", tmp_path / "missing-store"))
    assert_ok(services.ui_service("off", tmp_path / "missing-store"))


def test_missing_entrypoint_and_unsupported_platform_are_errors(content, home, manager, monkeypatch):
...
def test_launchd_status_reports_last_recorded_sync_when_no_run_time(content, manager):
    write(content / "machines/testmachine.json", '{"synced_at":"2026-09-05T01:20:00+00:00"}')
    report = services.timer("status", content)
    assert_ok(report)
    assert "last recorded sync: 2026-09-05T01:20:00+00:00" in str(report.lines())


def test_systemd_off_stops_an_inflight_sync_before_removing_units(content, home, manager, monkeypatch):
    monkeypatch.setattr(services.platform, "system", lambda: "Linux")
    assert_ok(services.timer("on", content))
...
def test_failed_stop_keeps_systemd_service_files_for_recovery(content, home, manager, monkeypatch):
    monkeypatch.setattr(services.platform, "system", lambda: "Linux")
    assert_ok(services.timer("on", content))
    manager.loaded.add("agent-hub-sync.service")
    manager.fail = "stop"
...
@pytest.mark.parametrize("payload,expected", [(b'{"repo":"/unrelated/store"}', False), (b'{}', False), (b'not JSON', False)])
def test_ui_probe_rejects_other_http_servers(content, monkeypatch, payload, expected):
    import io
    response = io.BytesIO(payload)
    response.status = 200
    monkeypatch.setattr(services.urllib.request, "urlopen", lambda *args, **kwargs: response)
